...
void CGameObject::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	//
	//m_pd3dcbObject->Map(0, NULL, (void**)&m_pcbMappedObject);
}

void CGameObject::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	//pd3dCommandList->SetGraphicsRootConstantBufferView(0, m_pd3dcbObject->GetGPUVirtualAddress());
}
...
CBullet::CBullet(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
	ID3D12RootSignature* pd3dGraphicsRootSignature, XMFLOAT3 xmf3Position)
	:CGameObject(1)
{
	m_State = BULLET_STATE::Wait;
	SetPosition(xmf3Position);
	...
	m_BoundingBox = pCubeMesh->GetBoundingBox();
	...
	pBulletShader->CreateCbvSrvDescriptorHeaps(pd3dDevice, 0, 1);
	pBulletShader->CreateShaderResourceViews(pd3dDevice, pBulletTexture, 0, 9);
...
{
	if (m_State != BULLET_STATE::Fired) return;
...
void CBullet::Fire(XMFLOAT3 xmf3Position)
{ 
	m_State = BULLET_STATE::Fired;
	SetPosition(xmf3Position);
} 
...
{
	XMFLOAT3 xmf3Position = GetPosition();
	xmf3Position.x += m_xmf3Direction.x * BULLET_SPEED * fTimeElapsed;
	xmf3Position.y += m_xmf3Direction.y * BULLET_SPEED * fTimeElapsed;
	xmf3Position.z += m_xmf3Direction.z * BULLET_SPEED * fTimeElapsed;
	SetPosition(xmf3Position);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
//
CExplosion::CExplosion(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, 
	ID3D12RootSignature* pd3dGraphicsRootSignature, XMFLOAT3 xmf3Position) 
	:CGameObject(1)
{
	m_State = OBJECT_STATE::Waiting;
	CGeometryBillboardVertex* pGeometryBillboardVertices = new CGeometryBillboardVertex();
	 
	float fyOffset = 16.0f * 0.5f;
	XMFLOAT2 xmf2Size = XMFLOAT2(8.0f, 16.0f);
	UINT nTexture = 0; 

	SetPosition(xmf3Position);
	pGeometryBillboardVertices->m_xmf3Position = xmf3Position;
	pGeometryBillboardVertices->m_xmf2Size = xmf2Size;
	pGeometryBillboardVertices->m_nTexture = 0;
	 
	for (int i = 0; i < 6; ++i)
	{
		pGeometryBillboardVertices->m_nTexture = i;
		m_pMeshes.push_back(new CGeometryBillboardMesh(pd3dDevice, pd3dCommandList, pGeometryBillboardVertices, 1));
	}


	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	 
	CTexture* m_pBillboardTexture = new CTexture(6, RESOURCE_TEXTURE2D, 0, 1);
  
	m_pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Explosion_0.dds", RESOURCE_TEXTURE2D, 0);
	m_pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Explosion_1.dds", RESOURCE_TEXTURE2D, 1);
	m_pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Explosion_2.dds", RESOURCE_TEXTURE2D, 2);
	m_pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Explosion_3.dds", RESOURCE_TEXTURE2D, 3);
	m_pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Explosion_4.dds", RESOURCE_TEXTURE2D, 4);
	m_pBillboardTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Explosion_5.dds", RESOURCE_TEXTURE2D, 5);

	CExplosionShader* pExplosionShader = new CExplosionShader();
	pExplosionShader->CreateShader(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT);
	pExplosionShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);

	pExplosionShader->CreateCbvSrvDescriptorHeaps(pd3dDevice, 0, 6);
	pExplosionShader->CreateShaderResourceViews(pd3dDevice, m_pBillboardTexture, 0, 10);
	
	CMaterial* pExplosionMaterial = new CMaterial();
	pExplosionMaterial->SetTexture(m_pBillboardTexture);
	pExplosionMaterial->SetShader(pExplosionShader);
	
	SetMaterial(0, pExplosionMaterial);
	
	if (pGeometryBillboardVertices) pGeometryBillboardVertices;
}
...
void CExplosion::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	if (m_State != OBJECT_STATE::Animating) return;
	...
	if (m_nMaterials > 0)
	{
		for (int i = 0; i < m_nMaterials; i++)
		{
			...
			m_pMeshes[m_nAnimaionIndex]->Render(pd3dCommandList);
...
void CExplosion::Update(float fTimeElapsed)
{
	if (m_State != OBJECT_STATE::Animating) return;

	m_fAnimationTimeSum += fTimeElapsed;

	if (m_fAnimationTimeSum > 0.3f)
	{
		m_nAnimaionIndex += 1;
		if (m_nAnimaionIndex >= 5) m_nAnimaionIndex = 0;
		m_fAnimationTimeSum = 0.0f;
	}
