...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	// Direct3D디바이스, 명령 큐와 명령 리스트, 스왑체인 등을 생성하는 함수를 호출한다.
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	// 모든 하드웨어 어대버에 대하여 특성 레벨 12.0을 지원하는 하드웨어 디바이스를 생성한다.
	...
	d3dMsaaQualityLevels.SampleCount = 4; // Msaa4x 다중샘플링
	...
	// 다중 샘플의 품질 수준이 1도마 크면 다중 샘플링을 활성화한다.
...
void CGameFramework::FrameAdvance()
{
	...
	d3dResourceBarrier.Transition.StateBefore =
		D3D12_RESOURCE_STATE_PRESENT;
	d3dResourceBarrier.Transition.StateAfter = 
	...
	(D3D12_RESOURCE_STATE_PRESENT)에서 렌더 타겟 상태
	(D3D12_RESOURCE_STATE_RENDER_TARGET)으로 바뀔 것이다.
	*/
	...
	m_pd3dCommandList->ClearRenderTargetView(
		d3dRtvCPUDescriptorHandle, pfClearColor, 0, NULL);
	...
	GPU가 렌더 타겟(버퍼)을 더 이상 사용하지 않으면 렌더 타겟의
	상태는 프리젠트 상태(D3D12_RESOURCE_STATE_PRESENT)로 바뀔 것 이다.
	*/
	...
	m_pd3dCommandQueue->ExecuteCommandLists(
		_countof(ppd3dCommandLists), ppd3dCommandLists);
	// 명령 리스트를 큐에 추가하여 실행한다.
	...
	*/
	...
	m_pszBuffer문자열이 "LabProject ("으로 초기화되었으므로
	m_pszFrameRate+12에서부터 프레임레이트를 문자열로 출력하여
	" FPS)"문자열과 합친다.*/
}
...
void CGameFramework::WaitForGpuComplete()
{
	...
	// GPU가 펜스의 값을 설정하는 명령을 큐에 추가한다.

	if (m_pd3dFence->GetCompletedValue() < nFence)
	{
		// 펜스의 현재 값이 설정한 값보다 작으면 펜스의 현재 값이
		// 설정한 값이 될 때까지 기다린다.
