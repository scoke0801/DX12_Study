...
private:
	...
	UINT m_nMsaa4xQualityLevels = 0; // MSAA 다중샘플링을 활성화하고 다중샘플링 레벨을 설정한다.

	static const UINT m_nSwapChainBuffers = 2;	// 스왑 체인의 후면 버퍼의 개수이다.
	UINT m_nSwapChainBufferIndex = 0;		// 현재 스왑체인의 후면 버퍼 인덱스이다.
	...
	ID3D12Resource* m_ppd3dDepthStencilBuffer;
	ID3D12DescriptorHeap* m_pd3dDsvDescriptorHeap;
	UINT m_nDsvDescriptorIncrementSize;
	// 깊이-스텐실 버퍼, 서술자 힙 인터페이스 포인터, 깊이-스테실 서술자 원소의 크기이다.
...
public:
	...
	bool OnCreate(HINSTANCE hInstance, HWND hMainWnd);	// 프레임워클르 초기화하는 함수이다(주 윈도우가 생성되면 호출된다)
	void OnDestroy();
