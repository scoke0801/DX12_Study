...
void GS_Main(point VS_OUT input[1], inout TriangleStream<GS_OUT> outputStream)
{
	...
	// View Space
	output[0].position = vertex.viewPos + float4(-scale, scale, 0.f, 0.f);
	output[1].position = vertex.viewPos + float4(scale, scale, 0.f, 0.f);
	output[2].position = vertex.viewPos + float4(scale, -scale, 0.f, 0.f);
	output[3].position = vertex.viewPos + float4(-scale, -scale, 0.f, 0.f);

	// Projection Space
	output[0].position = mul(output[0].position, g_matProj);
	output[1].position = mul(output[1].position, g_matProj);
	output[2].position = mul(output[2].position, g_matProj);
	output[3].position = mul(output[3].position, g_matProj);
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
	...
	float deltaTime = g_vec_1.x;
	float accTime = g_vec_1.y;
